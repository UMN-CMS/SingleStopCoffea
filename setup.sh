...
function startup_with_container(){
    ...
    if [ "$in_apptainer"  = false ]; then
        ...
        apptainer exec --pid --ipc --contain --bind /tmp --bind /cvmfs --bind /etc/hosts --home .:/srv --pwd /srv "$CONTAINER" /bin/bash --rcfile <(echo 'source setup.sh lcg')
    else
        printf "Currently in apptainer. Setting up\n"
        startup_with_lcg
    fi
}
