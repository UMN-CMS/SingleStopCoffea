...
class DatasetPreprocessed:
    ...
    def fromDatasetInput(dataset_input, **kwargs):
        out, x = dst.preprocess(dataset_input.coffea_dataset, save_form=False, uproot_options=dict(timeout=120), skip_bad_files=False, **kwargs)
...
def preprocessBulk(dataset_input: Iterable[AnalyzerInput], **kwargs):
    ...
    out, x = dst.preprocess(all_inputs, uproot_options=dict(timeout=120), skip_bad_files=False, **kwargs)
